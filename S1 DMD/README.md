...
1. Go to `update_settings.m` in matlab (located in `PredSim-workshop-ESMAC-2026/code`) and add the setting `S.subject.scale_MT_params` to scale the tendon slack length (lTs) in order to simulate a Achilles tendon lengthening. Specifically, copy the code below into `update_settings.m`. This line will scale the tendon slack length (lTs) of both muscles (left and right) by 1.3 :
